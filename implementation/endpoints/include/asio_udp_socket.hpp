...
public:
    asio_udp_socket(boost::asio::io_context& _io) : socket_(std::make_shared<boost::asio::ip::udp::socket>(_io)) { }
...
private:
    ...
    void async_connect(boost::asio::ip::udp::endpoint const& remote, completion_handler handler) override {
        auto socket = socket_;
        socket->async_connect(remote, [f = std::move(handler), socket](auto const& _ec) { f(_ec); });
    }
    void async_receive_from(boost::asio::mutable_buffer b, boost::asio::ip::udp::endpoint& remote, rw_handler handler) override {
        auto socket = socket_;
        socket->async_receive_from(b, remote, [f = std::move(handler), socket](auto const& _ec, size_t _bytes) { f(_ec, _bytes); });
    }
    void async_send(boost::asio::const_buffer const& b, rw_handler handler) override {
        auto socket = socket_;
        socket->async_send(b, [f = std::move(handler), socket](auto const& _ec, size_t _bytes) { f(_ec, _bytes); });
    }
    void async_send_to(boost::asio::const_buffer const& b, boost::asio::ip::udp::endpoint destination, rw_handler handler) override {
        auto socket = socket_;
        socket->async_send_to(b, destination, [f = std::move(handler), socket](auto const& _ec, size_t _bytes) { f(_ec, _bytes); });
    }
