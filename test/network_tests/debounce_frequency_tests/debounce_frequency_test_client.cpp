...
test_client::test_client(const char* app_name_) : vsomeip_utilities::base_vsip_app(app_name_) {
    ...
    _app->register_message_handler(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, vsomeip::ANY_EVENT,
                                   std::bind(&test_client::on_message, this, std::placeholders::_1));
    _app->request_event(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_EVENT, {DEBOUNCE_EVENTGROUP}, vsomeip::event_type_e::ET_FIELD,
                        vsomeip::reliability_type_e::RT_UNRELIABLE);
    _app->request_event(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_EVENT_2, {DEBOUNCE_EVENTGROUP}, vsomeip::event_type_e::ET_FIELD,
                        vsomeip::reliability_type_e::RT_UNRELIABLE);
    _app->request_service(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_MAJOR, DEBOUNCE_MINOR);
    _app->subscribe(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_EVENTGROUP, DEBOUNCE_MAJOR, DEBOUNCE_EVENT);
    _app->subscribe(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_EVENTGROUP, DEBOUNCE_MAJOR, DEBOUNCE_EVENT_2);
}
...
void test_client::send_request() {
    ...
    if (condition_availability.wait_for(lk, common::scaled_timeout(std::chrono::seconds(20)), [this] { return availability; })) {
        ...
        auto its_message = vsomeip_utilities::create_standard_vsip_request(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_START_METHOD,
                                                                           DEBOUNCE_MAJOR, vsomeip::message_type_e::MT_REQUEST_NO_RETURN);
...
void test_client::stop_service() {
    auto its_message = vsomeip_utilities::create_standard_vsip_request(DEBOUNCE_SERVICE, DEBOUNCE_INSTANCE, DEBOUNCE_STOP_METHOD,
                                                                       DEBOUNCE_MAJOR, vsomeip::message_type_e::MT_REQUEST_NO_RETURN);
    _app->send(its_message);
}
...
TEST(debounce_frequency_test, client) {
    ...
    ASSERT_EQ(debounce_client.was_event1_recv(), 1) << "Event 1 expected to be received once by the client, instead it was received "
                                                    << debounce_client.was_event1_recv() << " times.";
    ASSERT_EQ(debounce_client.was_event2_recv(), 1) << "Event 2 expected to be received once by the client, instead it was received "
