...
public:
    event_test_client(struct event_test::service_info _service_info, event_test::test_mode_e _mode, bool _use_tcp) :
        service_info_(_service_info), test_mode_(_mode), use_tcp_(_use_tcp),
        ...
        app_->register_state_handler(std::bind(&event_test_client::on_state, this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
                                       std::bind(&event_test_client::on_message, this, std::placeholders::_1));
        ...
        app_->register_availability_handler(
                ...
                std::bind(&event_test_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
        ...
        app_->register_subscription_status_handler(
                ...
                std::bind(&event_test_client::on_subscription_status_changed, this, std::placeholders::_1, std::placeholders::_2,
    ...
    ~event_test_client() { send_thread_.join(); }
    ...
    void on_notification(const std::shared_ptr<vsomeip::message>& _message) {
        ...
        VSOMEIP_DEBUG << "Received a notification with Client/Session [" << std::hex << std::setfill('0') << std::setw(4)
                      ...
                      << std::setw(4) << _message->get_service() << "/" << std::setw(4) << _message->get_method() << "]";
    }
    ...
    void send() {
        {
            ...
            its_payload->set_data(std::vector<vsomeip::byte_t>(
                    {static_cast<vsomeip::byte_t>(test_mode_), static_cast<vsomeip::byte_t>(number_events_to_send_)}));
