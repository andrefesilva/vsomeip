...
payload_test_client::payload_test_client(bool _use_tcp, bool _call_service_sync, std::uint32_t _sliding_window_size) :
    ...
    number_of_sent_messages_(0), number_of_sent_messages_total_(0), number_of_acknowledged_messages_(0), current_payload_size_(1),
    all_msg_acknowledged_(false), sender_(std::bind(&payload_test_client::run, this)) { }
...
bool payload_test_client::init() {
    ...
    app_->register_state_handler(std::bind(&payload_test_client::on_state, this, std::placeholders::_1));

    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip_test::TEST_SERVICE_INSTANCE_ID, vsomeip::ANY_METHOD,
                                   std::bind(&payload_test_client::on_message, this, std::placeholders::_1));

    app_->register_availability_handler(
            vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
            std::bind(&payload_test_client::on_availability, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    return true;
}
...
void payload_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
...
void payload_test_client::run() {
    ...
    if (shutdown_service_at_end) {
        shutdown_service();
        // Wait for the service to become unavailable. The wait must be done
        // before clear_all_handler() removes the availability handler.
...
void payload_test_client::send_messages_sync(std::unique_lock<std::mutex>& lk) {
    for (number_of_sent_messages_ = 0; number_of_sent_messages_ < number_of_messages_to_send_;
        ...
        all_msg_acknowledged_cv_.wait(lk, [&] { return all_msg_acknowledged_; });
...
void payload_test_client::print_throughput() {
    ...
    std::double_t mbyte_per_sec =
            ((number_of_sent_messages_ * current_payload_size_) / (static_cast<double>(time_needed) / usec_per_sec)) / (1024 * 1024);

    VSOMEIP_INFO << "[ Payload Test ] : :" << std::dec << std::setfill('0') << "Payload size [byte]: " << std::setw(8)
                 << current_payload_size_ << " Messages sent: " << std::setw(8) << number_of_sent_messages_
                 << " Meantime/message [usec]: " << std::setw(8) << time_per_message << " Calls/sec: " << std::setw(8) << calls_per_sec
                 << " MiB/sec: " << std::setw(8) << mbyte_per_sec;
}
    ...
    while (i < argc) {
        if (tcp_enable == argv[i]) {
            ...
            VSOMEIP_INFO << "Parameters:\n"
                         ...
                         << "--number-of-messages: Number of messages to send per payload size "
                            "iteration\n"
