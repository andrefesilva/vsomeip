...
public:
    ...
    bool init() {
        ...
        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id, cpu_load_test::method_id,
                                       std::bind(&cpu_load_test_service::on_message, this, std::placeholders::_1));

        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id, cpu_load_test::method_id_shutdown,
                                       std::bind(&cpu_load_test_service::on_message_shutdown, this, std::placeholders::_1));
        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id, cpu_load_test::method_id_cpu_measure_start,
                                       std::bind(&cpu_load_test_service::on_message_start_measuring, this, std::placeholders::_1));
        app_->register_message_handler(cpu_load_test::service_id, cpu_load_test::instance_id, cpu_load_test::method_id_cpu_measure_stop,
                                       std::bind(&cpu_load_test_service::on_message_stop_measuring, this, std::placeholders::_1));
        app_->register_state_handler(std::bind(&cpu_load_test_service::on_state, this, std::placeholders::_1));
        return true;
    }
    ...
    void on_message_stop_measuring(const std::shared_ptr<vsomeip::message>& _request) {
        (void)_request;
        load_measurer_.stop();
        VSOMEIP_DEBUG << "Received " << std::setw(4) << std::setfill('0') << number_of_received_messages_
    ...
    void on_message_shutdown(const std::shared_ptr<vsomeip::message>& _request) {
        ...
        VSOMEIP_INFO << "Received: " << number_of_received_messages_total_
                     << " in total (excluding control messages). This caused: " << std::fixed << std::setprecision(2) << average_load
                     << "% load in average (average of " << results_.size() << " measurements).";
