...
enum protocol_e { PR_UNKNOWN, PR_TCP, PR_UDP };
...
public:
    cpu_load_test_client(protocol_e _protocol, std::uint32_t _number_of_calls, std::uint32_t _payload_size, bool _call_service_sync,
        ...
        shutdown_service_at_end_(_shutdown_service), sliding_window_size_(_number_of_calls), wait_for_availability_(true),
        is_available_(false), number_of_calls_(_number_of_calls), number_of_calls_current_(0), number_of_sent_messages_(0),
        number_of_sent_messages_total_(0), number_of_acknowledged_messages_(0), payload_size_(_payload_size),
        wait_for_all_msg_acknowledged_(true), initialized_(false), sender_(std::bind(&cpu_load_test_client::run, this)) {
        if (!app_->init()) {
            ADD_FAILURE() << "Couldn't initialize application";
            return;
        }
        initialized_ = true;
        app_->register_state_handler(std::bind(&cpu_load_test_client::on_state, this, std::placeholders::_1));

        app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip::ANY_INSTANCE, vsomeip::ANY_METHOD,
                                       std::bind(&cpu_load_test_client::on_message, this, std::placeholders::_1));

        app_->register_availability_handler(cpu_load_test::service_id, cpu_load_test::instance_id,
                                            std::bind(&cpu_load_test_client::on_availability, this, std::placeholders::_1,
...
private:
    ...
    void on_state(vsomeip::state_type_e _state) {
        if (_state == vsomeip::state_type_e::ST_REGISTERED) {
            app_->request_service(cpu_load_test::service_id, cpu_load_test::instance_id);
        }
    }
    ...
    void run() {
        ...
        VSOMEIP_INFO << "Sent: " << number_of_sent_messages_total_
                     ...
                     << average_load << "% load in average (average of " << results_.size() << " measurements).";
        ...
        const double average_load_no_zero(std::accumulate(results_no_zero.begin(), results_no_zero.end(), 0.0)
    ...
    void send_messages_sync(std::unique_lock<std::mutex>& lk, std::uint32_t _messages_to_send) {
        ...
        VSOMEIP_DEBUG << "Synchronously sent " << std::setfill('0') << std::setw(4) << number_of_sent_messages_
                      << " messages. CPU load [%]: " << std::fixed << std::setprecision(2)
    ...
    void send_messages_async(std::unique_lock<std::mutex>& lk, std::uint32_t _messages_to_send) {
        ...
        c.stop();
        send_service_start_measuring(false);
        VSOMEIP_DEBUG << "Asynchronously sent " << std::setw(4) << std::setfill('0') << number_of_sent_messages_
                      << " messages. CPU load [%]: " << std::fixed << std::setprecision(2)
                      << (std::isfinite(c.get_cpu_load()) ? c.get_cpu_load() : 0.0);
        results_.push_back(std::isfinite(c.get_cpu_load()) ? c.get_cpu_load() : 0.0);
    }
    ...
    void send_service_start_measuring(bool _start_measuring) {
        ...
        _start_measuring ? m->set_method(cpu_load_test::method_id_cpu_measure_start)
                         : m->set_method(cpu_load_test::method_id_cpu_measure_stop);
        app_->send(m);
    }
...
int main(int argc, char** argv) {
    ...
    while (i < argc) {
        if (std::string("--protocol") == std::string(argv[i]) || std::string("-p") == std::string(argv[i])) {
            if (std::string("udp") == std::string(argv[i + 1]) || std::string("UDP") == std::string(argv[i + 1])) {
                protocol = protocol_e::PR_UDP;
                i++;
            } else if (std::string("tcp") == std::string(argv[i + 1]) || std::string("TCP") == std::string(argv[i + 1])) {
            ...
            i++;
        } else if (std::string("--mode") == std::string(argv[i]) || std::string("-m") == std::string(argv[i])) {
            if (std::string("sync") == std::string(argv[i + 1]) || std::string("SYNC") == std::string(argv[i + 1])) {
                call_service_sync = true;
                i++;
            } else if (std::string("async") == std::string(argv[i + 1]) || std::string("ASYNC") == std::string(argv[i + 1])) {
            ...
            i++;
        } else if (std::string("--help") == std::string(argv[i]) || std::string("-h") == std::string(argv[i])) {
