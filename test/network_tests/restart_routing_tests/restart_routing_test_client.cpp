...
restart_routing_test_client::restart_routing_test_client() :
    app_(vsomeip::runtime::get()->create_application()), is_available_(false), received_responses_(0) { }
...
void restart_routing_test_client::on_state(vsomeip::state_type_e _state) {
...
void restart_routing_test_client::on_availability(vsomeip::service_t _service, vsomeip::instance_t _instance, bool _is_available) {
...
void restart_routing_test_client::on_message(const std::shared_ptr<vsomeip::message>& _response) {
    ...
    if (_response->get_service() == vsomeip_test::TEST_SERVICE_SERVICE_ID
        ...
        if (received_responses_ == vsomeip_test::NUMBER_OF_MESSAGES_TO_SEND_ROUTING_RESTART_TESTS) {
            ...
            all_responses_received_ = true;
            condition_.notify_all();
        }
    }
}

void restart_routing_test_client::init() {
    if (!app_->init()) {
        ADD_FAILURE() << "Couldn't initialize application";
        return;
    }

    app_->register_state_handler(std::bind(&restart_routing_test_client::on_state, this, std::placeholders::_1));
    app_->register_message_handler(vsomeip::ANY_SERVICE, vsomeip_test::TEST_SERVICE_INSTANCE_ID, vsomeip::ANY_METHOD,
                                   std::bind(&restart_routing_test_client::on_message, this, std::placeholders::_1));
    app_->register_availability_handler(vsomeip_test::TEST_SERVICE_SERVICE_ID, vsomeip_test::TEST_SERVICE_INSTANCE_ID,
                                        std::bind(&restart_routing_test_client::on_availability, this, std::placeholders::_1,
...
void restart_routing_test_client::send_messages() {
    {
        std::scoped_lock lock{mutex_};
        sending_status_ = sending_status::SEND_MESSAGES;
    }
    condition_.notify_all();
}
