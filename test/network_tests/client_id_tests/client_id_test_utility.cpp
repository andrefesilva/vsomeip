...
public:
    client_id_utility_test() : client_id_routing_manager_(0x0), diagnosis_(0x0), diagnosis_mask_(0xFF00), client_id_base_(0x0) {
...
protected:
    virtual void SetUp() {
        ...
        CLIENT_ID_ROUTING_MANAGER =
                static_cast<client_t>((configuration_->get_diagnosis_address() << 8) & configuration_->get_diagnosis_mask());
        ...
        rm_impl_thread_ = std::thread([&]() { app_->start(); });
...
public:
    ...
    std::shared_ptr<vsomeip::application> get_app() { return app_; }
...
TEST_F(client_id_utility_test, ensure_preconfigured_client_ids_not_used_for_autoconfig) {
    ...
    for (int i = 0; i < limit; i++) {
        its_apps.emplace_back(std::make_shared<app_wrapper>(APPLICATION_NAME_NOT_PREDEFINED + std::to_string(i)));
    }
...
TEST_F(client_id_utility_test, request_predefined_client_id_in_diagnosis_range) {
    auto app1 = std::make_shared<app_wrapper>(APPLICATION_IN_NAME);
    EXPECT_EQ(APPLICATION_IN_CLIENT_ID, app1->get_client());
}

TEST_F(client_id_utility_test, request_predefined_client_id_in_diagnosis_range_twice) {
    auto app1 = std::make_shared<app_wrapper>(APPLICATION_IN_NAME);
    EXPECT_EQ(APPLICATION_IN_CLIENT_ID, app1->get_client());
...
TEST_F(client_id_utility_test, request_predefined_client_id_outside_diagnosis_range_high) {
...
TEST_F(client_id_utility_test, request_client_id_with_predefined_app_name_outside_diagnosis_range_high_multiple) {
...
TEST_F(client_id_utility_test, request_predefined_client_id_outside_diagnosis_range_low) {
    auto app1 = std::make_shared<app_wrapper>(APPLICATION_OUT_LOW_NAME);
    // we should get the client ID defined in the json file
    EXPECT_EQ(APPLICATION_OUT_LOW_CLIENT_ID, app1->get_client());
}

TEST_F(client_id_utility_test, request_predefined_client_id_outside_diagnosis_range_low_multiple) {
    auto app1 = std::make_shared<app_wrapper>(APPLICATION_OUT_LOW_NAME);
    // we should get the client ID defined in the json file
    EXPECT_EQ(APPLICATION_OUT_LOW_CLIENT_ID, app1->get_client());
...
TEST_F(client_id_utility_test, ensure_preconfigured_client_ids_in_diagnosis_range_dont_influence_autoconfig_client_ids) {
...
TEST_F(client_id_utility_test, exhaust_client_id_range_fragmented_extended) {
    ...
    // check limit is reached
    client_t its_illegal_client_id = utility::request_client_id(configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    ...
    its_illegal_client_id = utility::request_client_id(configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    EXPECT_EQ(VSOMEIP_CLIENT_UNSET, its_illegal_client_id);
...
TEST_F(client_id_utility_test, request_released_client_id_after_maximum_client_id_is_assigned) {
    ...
    client_t its_max_client_id = utility::request_client_id(configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    ...
    its_client_id = utility::request_client_id(configuration_, APPLICATION_NAME_NOT_PREDEFINED, VSOMEIP_CLIENT_UNSET);
    EXPECT_NE(VSOMEIP_CLIENT_UNSET, its_client_id);
    its_client_ids.push_back(its_client_id);
