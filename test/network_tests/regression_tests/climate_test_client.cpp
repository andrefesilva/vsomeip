...
public:
    client_sample(struct climate_test::service_info _service_info) :
        app_(vsomeip::runtime::get()->create_application()), service_info_(_service_info) { }
    ...
    void on_message(const std::shared_ptr<vsomeip::message>& _response) {
        ...
        its_message << "Received a notification for Event [" << std::hex << std::setfill('0') << std::setw(4) << _response->get_service()
                    << "." << std::setw(4) << _response->get_instance() << "." << std::setw(4) << _response->get_method()
                    << "] to Client/Session [" << std::setw(4) << _response->get_client() << "/" << std::setw(4) << _response->get_session()
                    << "] = ";
        std::shared_ptr<vsomeip::payload> its_payload = _response->get_payload();
        EXPECT_EQ(its_payload->get_length(), 5);
        its_message << "(" << std::dec << its_payload->get_length() << ") ";
        its_message << std::hex << std::setfill('0');
