...
void npdu_test_rmd::init() {
    ...
    app_->register_message_handler(npdu_test::RMD_SERVICE_ID_SERVICE_SIDE,
                                   ...
                                   std::bind(&npdu_test_rmd::on_message_shutdown, this, std::placeholders::_1));

    app_->register_state_handler(std::bind(&npdu_test_rmd::on_state, this, std::placeholders::_1));
}
...
void npdu_test_rmd::on_message_shutdown(const std::shared_ptr<vsomeip::message>& _request) {
    (void)_request;
    std::shared_ptr<vsomeip::message> request = vsomeip::runtime::get()->create_request(false);
#ifdef RMD_CLIENT_SIDE
    static uint32_t counter = 0;
    counter++;
    VSOMEIP_INFO << counter << " of " << npdu_test::client_ids_clients.size() << " clients are finished.";
